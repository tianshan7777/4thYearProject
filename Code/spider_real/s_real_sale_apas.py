...
pages = [str(i) for i in range(1, 2286)]
...
for page in pages:
	#print('entre page')
	#Make a request
	response = get('https://realestatehungary.hu/szukites/elado+lakas+budapest?page=' + page)
	...
	for container in containers:
		...
		if sub_page_html.find('div', class_ = 'paramterers') is not None:
			...
			if 'Elevator' in list_of_words:
				t = list_of_words[list_of_words.index('Elevator')+1].strip()
				#print('Elevator:', t)
				elevators.append(t)
				if t == 'available':
					lifts.append(1)
				elif t == 'not available':
					lifts.append(0)
				else:
					lifts.append(NOTFOUND)
			else:
			...
			if 'Heating' in list_of_words:
				...
				central_heating.append(NOTFOUND)
			...
			if 'Orientation' in list_of_words:
				...
				north.append(NOTFOUND)
				south.append(NOTFOUND)
				east.append(NOTFOUND)
				west.append(NOTFOUND)
				northeast.append(NOTFOUND)
				northwest.append(NOTFOUND)
				southeast.append(NOTFOUND)
				southwest.append(NOTFOUND)
			...
			if 'Furnished' in list_of_words:
				t = list_of_words[list_of_words.index('Furnished')+1]
				#print('Balcony:', t)
				if t == 'yes':
...
my_dict = {
	...
	'Heating: central heating': central_heating,
	#
...
#Clean data
print(properties.loc['Price(HUF)', : ])
properties.loc['Price(HUF)', : ] = properties.loc['Price(HUF)', : ].apply(lambda x:str(x)[ : -13].replace(' ', '')).astype(float)
print(properties.loc['Price(HUF)', : ])
...
#Produce a .csv file
properties.to_csv('realestatehungary_sale_apas.csv')
