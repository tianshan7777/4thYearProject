...
pages = [str(i) for i in range(1, 561)]
...
for page in pages:
	#print('entre page')
	#Make a request
	response = get('https://realestatehungary.hu/szukites/kiado+lakas+budapest?page=' + page)
...
properties.loc['Price(HUF)', : ] = properties.loc['Price(HUF)', : ].apply(lambda x:str(x)[ : -14].replace(' ', '')).astype(float)
print(properties.loc['Price(HUF)', : ])
...
#Produce a .csv file
properties.to_csv('realestatehungary_rent_apas.csv')
